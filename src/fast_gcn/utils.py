...
def get_expanded_graph(num_nodes, inward, type=1):
    A_expand = np.zeros((num_nodes + 6, num_nodes + 6),
                        dtype=np.float32)  # 25: upper_left,  26: upper_right,  27: up,  28: middle, 29: lower_left, 30: lower_right
    if type == 1:
        '''
...
class AdjMatrixGraph:
    def __init__(self, K=3, sample_size=5):
        ...
        self.A_binary = get_adjacency_matrix(neighbor, self.num_nodes)
        self.A_binary_with_I = get_adjacency_matrix(neighbor + self.self_loops, self.num_nodes)
        self.A = normalize_adjacency_matrix(self.A_binary_with_I)
